...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        wind_power: float = 15.0,
        turbulence_power: float = 1.5,
        distance_rew_weight: float = 1.0,
        angle_rew_weight: float = 1.0,
        velocity_rew_weight: float = 1.0,
        leg_rew_weight: float = 1.0,
    ):
        EzPickle.__init__(
            self,
            render_mode,
        ...
        self.distance_rew_weight = distance_rew_weight
        self.angle_rew_weight = angle_rew_weight
        self.velocity_rew_weight = velocity_rew_weight
        self.leg_rew_weight = leg_rew_weight
    ...
    def step(self, action):
        # adding success here to return if landing was successful
        ...
        reward = 0
        shaping = (
            - self.distance_rew_weight * 100 * np.sqrt(state[0] * state[0] + state[1] * state[1])  # distance to landing pad
            - self.velocity_rew_weight * 100 * np.sqrt(state[2] * state[2] + state[3] * state[3])  # velocity punishment
            - self.angle_rew_weight * 100 * abs(state[4])  # angle of lander
            + self.leg_rew_weight * 10 * state[6]  # leg touching the ground
            + self.leg_rew_weight * 10 * state[7]  # leg touching the ground
        ...
        if self.render_mode == "human":
            self.render()
        return np.array(state, dtype=np.float32), reward, terminated, False, {"success": success}
