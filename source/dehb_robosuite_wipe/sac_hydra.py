...
from sbx import SAC
from stable_baselines3.common.vec_env import VecMonitor, SubprocVecEnv, DummyVecEnv, VecVideoRecorder
from stable_baselines3.common.monitor import Monitor
from stable_baselines3.common.evaluation import evaluate_policy
from stable_baselines3.common.callbacks import BaseCallback

from robosuite.environments.manipulation.wipe import DEFAULT_WIPE_CONFIG
from envs.robosuite import TimeLimitWipe
...
class EvalTrainingMetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            mean_return, std_return = evaluate_policy(self.model, self.eval_env, n_eval_episodes=self.n_eval_episodes)
            ...
            if performance > self.best_performance:
                ...
                self.best_model_params = (self.model.policy.actor_state, self.model.policy.qf_state)
            if self.log_wandb:
    ...
    def _on_training_end(self) -> None:
        mean_return, std_return = evaluate_policy(self.model, self.eval_env, n_eval_episodes=self.n_eval_episodes)
        performance = self.return_weight * mean_return - self.stability_weight * std_return
        if performance > self.best_eval_return:
            self.best_performance = performance
            self.best_eval_return = mean_return
            self.best_eval_return_std = std_return
            self.best_model_params = (self.model.policy.actor_state, self.model.policy.qf_state)
...
def hydra_launcher(cfg, log) -> float:
    if cfg.search_space.random_reward_weights:
        ...
        cfg.algorithm.reward_shape['distance_multiplier'] = get_random_rshape(cfg, "distance_multiplier", rshape_rand_gen)
        cfg.algorithm.reward_shape['distance_th_multiplier'] = get_random_rshape(cfg, "distance_th_multiplier", rshape_rand_gen)
        cfg.algorithm.reward_shape['wipe_contact_reward'] = get_random_rshape(cfg, "wipe_contact_reward", rshape_rand_gen)
        cfg.algorithm.reward_shape['unit_wiped_reward'] = get_random_rshape(cfg, "unit_wiped_reward", rshape_rand_gen)
        cfg.algorithm.reward_shape['arm_limit_collision_penalty'] = get_random_rshape(cfg, "arm_limit_collision_penalty", rshape_rand_gen)
        cfg.algorithm.reward_shape['excess_force_penalty_mul'] = get_random_rshape(cfg, "excess_force_penalty_mul", rshape_rand_gen)
        cfg.algorithm.reward_shape['ee_accel_penalty'] = get_random_rshape(cfg, "ee_accel_penalty", rshape_rand_gen)
    ...
    task_config = DEFAULT_WIPE_CONFIG.copy()
    task_config['distance_multiplier'] = cfg.algorithm.reward_shape.distance_multiplier
    task_config['distance_th_multiplier'] = cfg.algorithm.reward_shape.distance_th_multiplier
    task_config['wipe_contact_reward'] = cfg.algorithm.reward_shape.wipe_contact_reward
    task_config['unit_wiped_reward'] = cfg.algorithm.reward_shape.unit_wiped_reward
    task_config['arm_limit_collision_penalty'] = cfg.algorithm.reward_shape.arm_limit_collision_penalty
    task_config['excess_force_penalty_mul'] = cfg.algorithm.reward_shape.excess_force_penalty_mul
    task_config['ee_accel_penalty'] = cfg.algorithm.reward_shape.ee_accel_penalty

    def make_train_env():
        def _init():
            return Monitor(TimeLimitWipe(config={
                "render_mode": None,
                "horizon": 500,
                "reward_shaping": True,
    ...
    train_env = VecMonitor(SubprocVecEnv([make_train_env() for _ in range(cfg.algorithm.num_envs)]))
    ...
    eval_env = VecMonitor(SubprocVecEnv([make_eval_env() for _ in range(cfg.algorithm.num_envs)]))
    eval_env.seed(cfg.seed+1)
    log_callback = LogAfterRollout(log_freq=500)
    ...
    log.info(f"Mean evaluation return across 20 episodes was {best_return} with std {best_return_std}")

    if cfg.algorithm.eval:
        model.policy.actor_state = best_model_params[0]
        model.policy.qf_state = best_model_params[1]

        eval_env.seed(cfg.seed+2)
        eval_mean_return, eval_std_return = evaluate_policy(model, eval_env, n_eval_episodes=128)
        ...
        default_env = VecMonitor(SubprocVecEnv([make_default_env() for _ in range(cfg.algorithm.num_envs)]))
...
@hydra.main(version_base="1.3", config_path="../config_robosuite_wipe", config_name="local_sac_runner")
