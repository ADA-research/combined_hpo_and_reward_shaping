...
def hydra_launcher(cfg, log) -> float:
    if cfg.algorithm.eval:
        ...
        cfg.algorithm.model_kwargs['num_evals'] = 10 // (5242881 // cfg.algorithm['num_timesteps'])  # Todo!!
    ...
    # normalize by summed reward and multiply with 6.35 (default summed rewards) to obtain same scale as hand-tuned reward-shaping
    # referred to as "explicit scaling" in the paper
    if not cfg.search_space.scale_reward:
        ...
        cfg.algorithm.reward_shape["forward_reward_weight"] = (cfg.algorithm.reward_shape["forward_reward_weight"] * 6.35) / summed_reward_shapes
        cfg.algorithm.reward_shape["healthy_reward"] = (cfg.algorithm.reward_shape["healthy_reward"] * 6.35) / summed_reward_shapes
        cfg.algorithm.reward_shape["ctrl_cost_weight"] = (cfg.algorithm.reward_shape["ctrl_cost_weight"] * 6.35) / summed_reward_shapes
    ...
    if cfg.algorithm.eval:
        ...
        default_env = brax.envs.get_environment(env_name="humanoid",backend=cfg.algorithm.backend)
...
@hydra.main(version_base="1.3", config_path="../config_brax_humanoid",
            config_name="local_sac_runner")
